...
ATE_lasso <- function(x.list.temp, B ) {
  X <- x.list.temp$X
  true.X <- X[,x.list.temp$true.cov]
  A <- x.list.temp$A
  Y <- x.list.temp$Y
  ps.df.org <- data.frame(A, X) 
  ps.df.org.m <- ps.df.org %>% as.matrix()
  # CV get lambda
  cv.output <- cv.glmnet(x = ps.df.org.m[,-1], y = ps.df.org.m[,1], nfolds = 10, family = "binomial", type.measure = "mse")
  best.lambda <- cv.output$lambda.min
  
  # Refit model with best lambda
  ps.ml <- glmnet(x = ps.df.org.m[,-1], y = ps.df.org.m[,1], lambda = best.lambda, family = "binomial")
  ...
  ps.df <- ps.df.org %>% 
    mutate(Y = as.numeric(Y),
           ps = predict(ps.ml, s = best.lambda, type = "response", newx = ps.df.org.m[,-1]),
           ps_quintile = cut(ps, 
  ...
  for(i in 1:B) {
    ...
    ps.ml.bt <- glmnet(x = ps.df.bt.m[,-1], y = ps.df.bt.m[,1], lambda = best.lambda, family = "binomial")
    # Obtain PS and Stratified
    ps.df.bt <- ps.df.bt %>% 
      mutate(Y = as.numeric(Y),
             ps = predict(ps.ml.bt, s = best.lambda, type = "response", newx = ps.df.bt.m[,-1]),
             ps_quintile = cut(ps, 
                               breaks = c(0, quantile(ps, p = c(0.2, 0.4, 0.6, 0.8 + small_jitter)), 1), 
                               labels = 1:5),
             w0 = (1-A)/(1-ps),
  ...
  # Return the ATE
  return(list(cov.info = data.frame(method = "lasso",
                                    n = nrow(X),
                                    m = ncol(X),
                                    s = ncol(true.X),
                                    ...
                                    miss.cov = miss.cov,
                                    add.cov = add.cov),
              result = data.frame(method = "lasso",
                                  n = nrow(X),
                                  m = ncol(X),
                                  s = ncol(true.X),
