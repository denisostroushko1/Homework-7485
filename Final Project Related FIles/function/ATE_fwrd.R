...
ATE_fwrd <- function(x.list.temp, B) {
  X <- x.list.temp$X
  true.X <- X[,x.list.temp$true.cov]
  A <- x.list.temp$A
  Y <- x.list.temp$Y
  ps.df.org <- data.frame(A, X)
  ps.ml.st <- glm(A ~ 1, data = ps.df.org, family = "binomial")
  ps.ml.end <- formula(glm(A ~ ., data = ps.df.org, family = "binomial")) 
  ...
  ps.ml <- glm(fwrd.formula, data = ps.df.org, family = "binomial")
  ...
  for(i in 1:B) {
    ...
    ps.ml.bt <- glm(fwrd.formula, data = ps.df.bt, family = "binomial")
    # Obtain PS and Stratified
    ps.df.bt <- ps.df.bt %>% 
      mutate(Y = as.numeric(Y),
