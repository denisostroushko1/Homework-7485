...
ATE_bkwrd <- function(x.list.temp, B) {
  X <- x.list.temp$X
  true.X <- X[,x.list.temp$true.cov]
  A <- x.list.temp$A
  Y <- x.list.temp$Y
  ps.df.org <- data.frame(A, X)
  ps.ml.st <- glm(A ~ ., data = ps.df.org, family = "binomial")
  bkwrd.res <- step(ps.ml.st, direction = "backward", trace = FALSE)
  bkwrd.formula <- bkwrd.res$formula
  
  ml.cov <- colnames(bkwrd.res$model[,-1])
  ml.cov <- as.numeric(gsub("X", "", ml.cov))
  ml.ord.idx <- order(ml.cov)
  
  ps.ml <- glm(bkwrd.formula, data = ps.df.org, family = "binomial")
  ...
  for(i in 1:B) {
    ...
    ps.ml.bt <- glm(bkwrd.formula, data = ps.df.bt, family = "binomial")
    # Obtain PS and Stratified
    ps.df.bt <- ps.df.bt %>% 
      mutate(Y = as.numeric(Y),
  ...
  # Return the ATE
  return(list(cov.info = data.frame(method = "backward selection",
                                    n = nrow(X),
                                    m = ncol(X),
                                    s = ncol(true.X),
                                    ...
                                    miss.cov = miss.cov,
                                    add.cov = add.cov),
              result = data.frame(method = "backward selection",
                                  n = nrow(X),
                                  m = ncol(X),
                                  s = ncol(true.X),
