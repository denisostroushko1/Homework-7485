...
```{r evaluation of intercation of the variabce estimates }
#| eval: false 
...
# previous version with some high VIF 
  ...
  model_1 = lm(log(MSE.ipw) ~ m + n + s*(method + p_true_selected) + p_false_selected, data = results) 
...
vif(model_1, type = 'predictor') %>% data.frame()
...
if(plots == "Covs by S"){
  ...
  plot_model(model = model_1, 
             type = "eff", 
             terms = c("p_false_selected[0, 1, 2]", "s[10, 20, 30]")) + 
    theme_minimal() + 
    theme(legend.position = "bottom") + 
...
plot_model(model = model_1, 
           type = "eff", 
           terms = c("p_false_selected[0, 1, 2]", "method", "s[10, 20, 30]" )) + 
  theme_minimal() + 
  theme(legend.position = "bottom")
...
1. As the number of true predictors increases, models get more complicated, and we have on average higeher MSE 

2. However, in a situatin where true predictor space is samll, adaptive lasso performs almost as good as the best case scenario of knowing the true predicotrs

3. As we go from simpler to more complicated space, all methods are good. Conclusion: use adaptive lasso 
