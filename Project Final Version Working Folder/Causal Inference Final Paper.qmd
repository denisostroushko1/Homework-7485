...
#### T-test - Naive ATE estiamtor 
In practice, some preliminary analyses will take the difference in average of obsrbed outcomes between the two groups without adjusting for confounding. It is a known established fact that this leads to a biased treatment effect estimation. We can also 
consider this method a 'null' variable selection, i.e. a method that performs no vairbale selection, and therefore 
does not perform any adjustment for existing confounders. 

Together with the 'Oracle' and 'Experience Based' methods, these three form a set of benchmarking methods. We will compare the 
performance of data driven methods with benchmarking methods in this study. 

```{r table methods}
#| labeL: tbl-descr
#| tbl-cap: "Variable Selection Methods Considered for Analysis. "

# Table Methods

tbl_methods <- data.frame(
  Methods = c(
    "Oracle",
    "No Variable Selection (T-test ATE Estimation)",
    "Experience Based",
    "Adaptive Lasso",
    "Lasso",
    "Forward Selection"
  ...
  Short_Description = c(
    ...
    "Selecting 5 correct and 5 randomly chosen predictors from the set of unrelated covariates",
...
kable(
  booktabs = T, 
  tbl_methods,
  col.names = c("Methods", "Short Description")
) %>%
...
@fig-unadjsuted-res illustrates and Mean Squared Error (MSE) for the various variable selection methods across varying model complexity scenarios. With growing number of true predictors,even simple linear additive models become complex. We expect that the task of constructing a regression model in such cases is a difficult task, which leads to potential of hugher bias, higher variance, or a steep trade off between the two. We introduced the two-sample t-test estimator as a benchmark for unadjusted Average Treatment Effect (ATE) estimation. It is acknowledged that neglecting confounders can lead to biased estimation, a fact visually confirmed in @fig-unadjsuted-res. Similarly, the 'Experience-Based' variable selection method yields estimates with high MSE, which is likely due to bias. Recall that the 'Experience-Based' method consistently selects five true and five random confounders, which leads to violation of 'No Unmeasured Confounders' assumption. 
...
```{r unadjsuted MSE }
#| label: fig-unadjsuted-res
#| fig-cap: "Mean Squared Error (MSE) directly from simulation results. Presented averages unadjusted for other potential factors explaining variations in MSE"
#| fig-width: 7
#| fig-height: 4
#| 
res.m %>% 
...
plot_d %>% 
  ggplot(aes(x = num, y = mean, group = s, color = s)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous(breaks = 1:length(unique(plot_d$method)), labels = unique(plot_d$method))+
  theme_bw() + 
  labs(y = "MSE", 
       x = "", 
       color = "Number of True Predictors", 
       title = "MSE of IPW ATE Estiamtor. Unadjsuted Simulation Results") + 
  theme(axis.text.x = element_text(angle = 90, 
                                   vjust  = 0.25))
```

```{r plot - summary of BIAS for IPW and PSS by method and covariate space size, eval = F }
...
@fig-unadjsuted-res shows that all data-driven methods exhibit a noticeable increase in MSE as the number of true confounders that need to be accounted for rises. We speculate that the rise in MSE and variance is a consequence of model misspecification.

Our speculation stems from the understanding that, under random sampling variability, it becomes increasingly challenging to select all true confounders as their number increases. Notably, for the Inverse Probability Weighting (IPW) class of Average Treatment Effect (ATE) estimators, model misspecification leads to higher variation in the estimator. The impact of this phenomenon is illustrated in the right graph in @fig-unadjsuted-res. 
...
```{r unadjsuted Percentages }
#| label: fig-unadjsuted-percentages
#| fig-cap: "% True and False predictors calculated based of the number of true predictors. Values over 100% imply that the number of false predicotrs selected exceeds the number of true covariates"
#| fig-width: 9
#| fig-height: 5
#| 
res.m %>% 
  group_by(method, s) %>% 
  arrange(s, method) %>%
  summarise(mean = mean(mean_p_true_selected), 
            mean_2 = mean(mean_p_total_selected)) %>% 
  ungroup()  %>% 
  na.omit() %>% 
  group_by(s) %>% 
  mutate(s = as.factor(s), 
...
plot_d$method <- factor(plot_d$method, 
                        levels = unique(as.character(plot_d$method) ) %>% unlist())
...
plot_d %>% 
  ...
  geom_line() + 
  scale_x_continuous(breaks = 1:length(unique(plot_d$method)), labels = unique(plot_d$method))+
  scale_y_continuous(labels  = function(x) paste0(round(x, 2) * 100, "%")) + 
  theme_bw() + 
  labs(y = "% True Predictors Selected", 
       x = "", 
       color = "Number of True Predictors", 
       title = "Average % of True Covariates Selected. Unadjsuted Simulation Results") + 
  theme(axis.text.x = element_text(angle = 90, 
                                   vjust  = 0.25)) -> p1 


plot_d %>% 
  ggplot(aes(x = num, y = mean_2, group = s, color = s)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous(breaks = 1:length(unique(plot_d$method)), labels = unique(plot_d$method))+
  scale_y_continuous(labels  = function(x) paste0(round(x, 2) * 100, "%")) + 
  theme_bw() + 
  labs(y = "% True Fasle Selected", 
       x = "", 
       color = "Number of True Predictors", 
       title = "Average % of True Covariates Selected. Unadjsuted Simulation Results") + 
  theme(axis.text.x = element_text(angle = 90, 
...
p1 + p2 + plot_layout(nrow = 1, guides = "collect") & theme(legend.position = "bottom") & 
  plot_annotation(title = 'Inlusion of True and False Predictors into Propensity Score Models ',
                  #caption = 'made with patchwork',
                  theme = theme(plot.title = element_text(size = 16)))
```

```{r plot - not valuated summary of MSE for IPW and PSS by method and covariate space size, eval = F }
#| eval: false 
...
@fig-unadjsuted-percentagesshows that on average, all data driven variable seelction methods to select about 75% to 85% of 
...
To assess the ability of each variable selection method to accurately specify the model, we examine the average proportion of true confounders selected and the average proportion of all predictors identified by the method in relation to the total number of true confounders. @fig-unadjsuted-percentages illustrates that, on average, all data-driven variable selection methods select approximately 75% to 85% of true confounders under varying data generation and sample size conditions. This higher percentage of correctly identified confounders is likely the primary factor contributing to low bias performance across all data generating scenarios. Notably, a steady deviation from this overall pattern is observed, indicating a consistent decrease in the number of true confounders as the number of options increases.

Conversely, @fig-unadjsuted-percentages also demonstrates that all data driven methods on average pick up big number of false positive covariates, likely due to sampling randomness. For example, when the number of true predictors is equal to 10, Lasso tends to pick up about 25 false positive predictors (i.e. 25 false positive / 10 true predictors = 250%). Additionally, on average, as the size of the true confounder space expands, the ratio of all selected covariates to the number of true confounders decreases for every method. Presumably, when the number of true confounders is small, each additional irrelevant predictor added to the model has a more substantial impact on this proportion. The likelihood of selecting a false positive predictor in the model-building process is expected to increase when the number of true confounders is small, driven by sampling variability.
...
```{r not - evaluated evaluation of interaction of the variabce estimates, eval = F }
#| eval: false 
...
predictors was below 50%. @fig-unadjsuted-percentagespresented in the previous section shows that the number of true covariates captured was 75% to 85% on average. Estiamtign effects and interpreting effects for the scenarios with sparsly presented data 
...
plot_model(model = model_1, 
  ...
  scale_x_continuous(breaks = c(0,0.5,1), 
                     labels = function(x){paste0(round(x,2)*100, "%")}) + 
  labs(y = "MSE IPW Estiamtor (Logarithmic Scale)", 
       x = "% True Covariates Selected", 
       title = "Variable selection method effect on MSE under varying model complexity conditions (10, 20 ,30 true preditors)", 
       color = "Variable Selection Method")
...
<!-- 
Our in depth analysis fo the variable selection method for IPW propoensity score model produced both expected and unexpected outcomes. THe purpose of this study was to find a mehtod that produces a model which leads to smallest degress of misspecification. We theorized that the smallest degreee of misspecification will lead to smaller variance and more precise estiamtors. The results section shows that the answer to this question depends on the underlying parameters, such as sample size and the complexity of the true model. We have shown that for a class of simpler additive logistic regression models, adaptive lasso regression model produces estimators with the lowest MSE among all considered data genererating and data collection scenarios. However, it is imporatnt to acknowdlege that the true model building is never know to researchers and analysysts. 
Therefore, based on the finding of the paper, we recommend that practicing staitiscians use an adaptive lasso regresison model for the IPW estiamtor. While we do not yet know how such a model compares with more flexible modeling techniques, in the 
context of a situation where interpretabke models are desired, adaptive lasso tends to produce best results in all unknown data generating sceantios that we consired. While for very complicated models (where the number of true predictors was 30) all data driven methods produced simialr resutls, the choice of variable selection model has little impact. However, in the situation where the number of true prediuctors is small, and the cost of including each additional false positive is high, the choice of variable selection mehotd becomes more cruicial. We want to emphasise one more time that the number of tru predicotrs in the underlying dta generating model will be unknown. Therefore, our recommendation is to use an adaptive lasso model for variable selection as it will guarantee the best performace of IPW estiamtor in terms of minimazing MSE, when compared to Lasso and Forward selection models. 
--> 
...
Our study also assessed the impact of true and false predictors selected on MSE. Due to interactions and compelx relationships we visualzue these marginal model effects using graphical tools in FGIURE 4 and FIGURE 5. It was unexpected to see that as 
the % of true covariates selected by each modethod increased, the MSE also increased. We specualte that increaseing model complexity leads to higher vairance of the estimator that depends on this model. In contrast, FIGURE 5 in appenxis shows the marginal impact of inclusion of false positive covaraites leads to no change in the MSE of the estimator. These findings can be comforting to a practicing statsitican, as the 'penalty' for inclusion of the false positive predictors does not seem be present. 
The focus is solely on the number of true positive predictors that need to be in the model. Since MSE inclorporates both variance and bias of the estimator, it is likely that the percentage of true predictors included, while holding percentage of false positive predicotrs at around 150% will result in a model with high number of predicotsts. Such a model may have low average bias due to accounting of many potetail confounders, however, sucha. model also will have a high degreee of variance in the estimates that it produces. 

While our analysis is not empowered to do so, a causal analysis may be performed in a future study that is based on these preliminary results.A futher simulation study may attempt to vary proportions of true positive and false positive predicots while holding other data generating parameters constant, and attempt to evaluate if MSE rate of change of MSE gets worse than the resutks that we presented. 

It is likely that the increase in MSE as the number of predictors grows is unavoidable, and the best we can do is to attemot to minimaize the rate at which MSE grows as we include more and more predictosrs. Such a finding may empower practicing analyst to approach their model building process with more knowledge of how their modeing choices will impact further inferences. 
--> 

Our investigation also delved into the influence of both true and false predictors on Mean Squared Error (MSE), visualizing these effects through graphical tools in @fig-effects-by-S and @fig-effects-by-S-2, considering interactions and complex relationships. Surprisingly, as the percentage of true covariates selected by each method increased, we observed a concurrent increase in MSE. Our speculation is that augmenting model complexity leads to higher variance in the estimator associated with this model.

In contrast, @fig-effects-by-S-2 in the appendix demonstrates that the marginal impact of including false positive covariates results in no discernible change in the MSE of the estimator. These findings may offer reassurance to practicing statisticians, indicating that the 'penalty' for the inclusion of false positive predictors appears to be absent. The emphasis lies solely on the presence of true positive predictors in the model.
...
# Conclusion 
<!-- 
In this study we evalauted the impact of variable selection process for the propensity score model in the constructuon IPW 
Average Treatment Effect Estiamtor. We consired varaying data generating scenariosn that aim to replicate real life data sets that reseachers and practiitoners use. The key factors of interest were the number of true predicots in the data generating mechanism, and the variabke sekection method. We found that on average all data driven methods tend to capture about 75% of true confoudners, while picking up aboutn 150% of false positive confoinders. We used these resutls, data generating conditions, and variable selection methods to constudct a Gaussian General Linear Model and study the impact of these factors on the Squared Errors on the logarithimic scale. We have found that MSE increases as the models get more compelx, and we advocate that the rate of MSE growth is evaluted in fither studies to see if it can be controled for useing differnt methods, of wether the effect is unavoidable, and the best we can do it to minimize it. Our key finding was that Adaptive lasso regression model that seelctes variable based on shringake parameters performs best among all data driven variable selection methods unders all known and unknown data genrating schemes. However, in the cases with a lot fo confoudners, all dta driven methods tend to perform similar, and converge to the best case scenario, on average. We advocate that in the scenario where interpretatble regression models are desired, the practitioner chooses Adaptive Lasso regression for model building purposes.
--> 
...
```{r}
tbl_MSE_sum_prep <- results %>%
  filter(method != "experience based", method != "PCA (80% of variance)") %>%
  mutate(MSE.pss = (1/n)*(ATE.true - ATE.pss)^2,   # calc MSE for each row
    MSE.ipw2 = (1/n)*(ATE.true - ATE.ipw2)^2) %>%
  group_by(method) %>%
  summarize(
    Bias.pss = (mean(bias.pss)),  # average by method
    MSE.pss.avg = mean(MSE.pss),
...
tbl_MSE_sum <- kable(
  tbl_MSE_sum_prep,
  caption = "Mean Squared Error of Adjusted Treatment Effect for Select Propensity Score Variable Selection Methods",
  digits = c(0, 4, 7, 3, 3, 3, 4, 7, 3, 3, 3),
  col.names = c("Method", "Bias", "MSE", "SE", "Lower", "Upper", "Bias", "MSE",
                "SE", "Lower", "Upper")
) %>%
  add_footnote("Average value of 500 simulated combinations of each combination of sample size (1500, 3000, 4500, 6000), potential covariates (50, 100, 150), and true covariates (10, 20, 30)") %>%
  add_header_above(header = c(" " = 4, "95% CI" = 2, " " = 3, "95% CI" = 2)) %>%
  add_header_above(header = c(" " = 1, "Propensity Score Stratification" = 5,
                              "Inverse Probability Weighting" = 5)) %>%
...
tbl_MSE_by_n <- kable(
  tbl_MSE_by_n_data,
  caption = "Mean Squared Error of Adjusted Treatment Effect for Select Propensity Score Variable Selection Methods",
  digits = c(0, 0, 4, 7, 3, 3, 3, 4, 7, 3, 3, 3),
  col.names = c("Method", "n", "Bias", "MSE", "SE", "Lower", "Upper", "Bias",
                "MSE", "SE", "Lower", "Upper")
) %>%
  add_footnote("Average value of 500 simulated combinations of each combination of potential covariates (50, 100, 150), and true covariates (10, 20, 30)") %>%
  add_header_above(header = c(" " = 2,
                              "Propensity Score Stratification" = 5,
                              "Inverse Probability Weighting" = 5)) %>%
  add_header_above(header = c(" " = 5, "95% CI" = 2, " " = 3, "95% CI" = 2)) %>%
  kable_styling(latex_options = c("scale_down", "HOLD_position"))
