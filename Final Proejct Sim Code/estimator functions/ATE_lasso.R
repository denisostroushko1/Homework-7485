...
ATE_lasso <- function(x.list.temp) {
  X <- x.list.temp$X
  ...
  ps.ml <- glmnet(x = ps.df.org.m[,-c(1,2)], y = ps.df.org.m[,2], lambda = best.lambda, family = "binomial")
  ml.cov <- which(ifelse(as.vector(ps.ml$beta) != 0, TRUE, FALSE))
  
  end_time <- Sys.time()
  execution_time <- end_time - start_time
  cat("Lasso execution time:", as.numeric(execution_time, units = "secs"), "secs(s)\n")
