...
ATE_truth <- function(x.list.temp) {
  X <- x.list.temp$X
  true.X <- X[,x.list.temp$true.cov]
  # randomly pick 80%
  # rnd.cov <- purrr::rbernoulli(ncol(true.X), p=0.8)
  
  pick.col <- ncol(true.X)
  
  rnd.cov.idx <- sample(1:ncol(true.X), pick.col*1)
  ...
  # Pick up noise
  rnd.cov.idx1 <- sample(1:(ncol(X) - ncol(true.X)), pick.col*0)
  ...
  cat("best case execution time:", as.numeric(execution_time, units = "secs"), "secs(s)\n")
