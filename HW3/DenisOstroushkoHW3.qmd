...
FactorVariables =  c(
    "Race_ethnicity", "Public.Asstce", "Use.Tob"
  )
...
### A ### 
p_a = predict(logistic_regression_a, data, type = "response")
# weight is TRT/PS + (1-TRT)/(1-PS)
weight_a <- data$num_treat_flag/p_a + (1 - data$num_treat_flag)/(1- p_a) 
...
### B ### 
p_b = predict(logistic_regression_b, data, type = "response")
weight_b <- data$num_treat_flag/p_b + (1 - data$num_treat_flag)/(1- p_b) 
...
weight_c <- data$num_treat_flag/p_c + (1 - data$num_treat_flag)/(1- p_c) 
...
### D ### 
p_d = predict(logistic_regression_d, data, type = "response")
weight_d <- data$num_treat_flag/p_d + (1 - data$num_treat_flag)/(1- p_d)
...
### RF ### 
p_e = rf$votes[,2]
weight_e <- I(data$Group == "C")/p_e + I(data$Group == "T")/(1- p_e)
...
ggplot(data = all_SMDs, 
  ...
  geom_hline(color = "red", linetype = "dashed", yintercept = 0.1) + 
  theme(
